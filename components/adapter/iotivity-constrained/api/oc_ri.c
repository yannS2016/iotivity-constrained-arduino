...
// we can save 20 bytes of GS by decalring this as uint8_t
static uint8_t oc_coap_status_codes[__NUM_OC_STATUS_CODES__];
...
#endif /* !OC_BLOCK_WISE */
{
  ...
  if (payload_len > 0) {
	...
	OC_WRN_V("ocri: error parsing request payload; tinyCBOR error code:  %d\n",
	parse_error);
#else
